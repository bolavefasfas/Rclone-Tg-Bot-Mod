...
async def mirrorset_menu(
    ...
    if submenu == "list_drive":
        ...
        buttons.cbl_buildbutton("✘ Close Menu", f"mirrorsetmenu^selfdest")
        ...
        buttons.cbl_buildbutton(f" ✅ Select this folder", f"mirrorsetmenu^selfdest")
        ...
        if return_code != 0:
           ...
           return await message.reply_text(f'Error: {err}', quote=True)  
        ...
        if len(list_info) == 0:
            ...
            if edit:
                return await message.edit(msg, buttons=buttons.first_button)
            else:
                return await message.reply(msg, buttons=buttons.first_button)
        else:    
            total = len(list_info)
            max_results= 10
            offset= 0
