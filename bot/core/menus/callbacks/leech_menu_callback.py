...
async def handle_leech_menu_callback(client, callback_query):
    ...
    if cmd[1] == "list_drive_leech_menu":
        ...
        rclone_leech= RcloneLeech(message, chat_id, base_dir, dest_dir, name, isZip=is_zip, extract=extract)
        await rclone_leech.execute()

    elif cmd[1] == "start_leech_folder":
        dest_dir = f'{DOWNLOAD_DIR}{base_dir}'
        rclone_leech= RcloneLeech(message, chat_id, base_dir, dest_dir, folder=True, isZip=is_zip, extract=extract)
        await rclone_leech.execute()
