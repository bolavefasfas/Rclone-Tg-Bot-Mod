...
async def next_page_copy(callback_query):
    ...
    if next_offset == 0:
        ...
        buttons.tbuildbutton(first_text="⏪ BACK", first_callback= "n_copy {prev_offset} {is_second_menu} {data_back_cb}",
                            second_text= f"🗓 {round(int(next_offset) / 10) + 1} / {round(total / 10)}", second_callback="setting pages",
                            third_text= "NEXT ⏩", third_callback=f"n_copy {next_offset} {is_second_menu} {data_back_cb}")
